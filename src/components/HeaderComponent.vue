        ...
        <div class="d-flex align-items-center justify-content-center ">
            <ul class="d-flex text-uppercase my-text-white m-0">
                <li class="list-unstyled px-4 py-5" v-for="(item, index) in this.store.nav" id="nav-list"
                    :class="{ 'text-title': store.scrolled }">
                    <h6 class="m-0">{{ item.name }}</h6>
                    <div class="sub-menu position-absolute"
                        :class="(item.submenu4.length > 0 ? 'my-width justify-content-center ' : '')">
                        <ul class="m-0 p-0" v-show="item.submenu1.length > 0">
                            <li class="list-unstyled" v-for="element in item.submenu1" id="sub-nav-list">
                                <div class="d-flex justify-content-between align-items-center position-relative">
                                    <a class="text-decoration-none my-text-white"
                                        :class="(element.title ? 'text-title text-uppercase' : '')" :href="element.url">
                                        {{ element.name }}
                                    </a>
                                    <span class="position-absolute chevron"
                                        v-show="element.name === 'Blog' || element.name === 'Forums' || element.name === 'Shop'">
                                        <i class="fa-solid fa-chevron-right"></i>
                                    </span>
                                </div>
                                <div class="sub-menu2 position-absolute"
                                    v-show="element.name === 'Forums' || element.name === 'Blog' || element.name === 'Shop'">
                                    <ul class="m-0 p-0">
                        ...
                        <ul class="m-0 p-0 my-border-left" v-show="item.submenu2.length > 0">
                            <li class="list-unstyled" v-for="element in item.submenu2">
                                <a class="text-decoration-none my-text-white" :href="element.url"
                                    :class="(element.title ? 'text-title text-uppercase' : '')">
                                    {{ element.name }}
                                </a>
                                ...
                                <div
                                    :class="{ 'blue-pill': element.price != 'Free' && element.prices, 'yellow-pill': element.price === 'Free' }">
                                    {{ element.price }}
                                </div>
                            </li>
                        </ul>
            ...
            <div class="my-text-white px-5 " :class="{ 'text-title': store.scrolled }">
